...
function getGameContextInfo(title = '🎮 SPI GAMES', body = 'Have fun playing!') {
...
function getRpgContextInfo(title = '⚔️ SPI RPG', body = 'Adventure awaits!') {
    const saluranId = config.saluran?.id || '120363208449943317@newsletter'
    const saluranName = config.saluran?.name || config.bot?.name || 'Si Paling Informasi'
...
function createFakeQuoted(botName = 'Si Paling Informasi', verified = true) {
    return {
        key: {
            fromMe: false,
