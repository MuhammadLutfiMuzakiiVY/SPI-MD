    ...
    switch (action.type) {
        ...
        case 'STICKER': {
            ...
            await sock.sendMessage(m.chat, {
                ...
                packname: config.bot?.name || 'Si Paling Informasi',
                author: 'AutoAI'
            }, { quoted: m })
...
async function chatWithOpenRouterFallback(fullMessage, fullInstruction, imageBuffer, history) {
    const keys = config.APIkey?.openrouter || {}

    // Coba Gemini HANYA jika key terkonfigurasi (menghindari lambatnya scraper)
    if (config.geminiApiKey) {
        try {
            console.log('[AutoAI] Trying Gemini API...')
            ...
            console.error('[AutoAI] Gemini failed, trying OpenRouter...', e.message)
        }
    }

    // List of models to try, starting with the verified fast one
    ...
    for (const model of fallbackModels) {
        ...
        try {
            console.log(`[AutoAI] Trying OpenRouter model: ${model}`)
            ...
            if (text) {
                console.log(`[AutoAI] Success with model: ${model}`)
                return text
            }
        } catch (err) {
            console.error(`[AutoAI] Model ${model} failed:`, err.response?.data?.error?.message || err.message)
        }
    }
...
async function handleAutoAI(m, sock) {
    ...
    if (!autoai) {
        db.db.data.autoai[m.chat] = {
            enabled: true,
            instruction: 'Kamu adalah Layanan Otomatis SPI, asisten virtual pintar yang sangat responsif, ramah, dan membantu.',
            responseType: 'text',
            sessions: {}
        }
        autoai = db.db.data.autoai[m.chat]
        db.save()
    }
    if (!autoai.enabled) return false
    ...
    try {
        ...
        let aiResponse = ''
        try {
            aiResponse = await chatWithOpenRouterFallback(fullMessage, fullInstruction, imageBuffer, history)
        } catch (apiError) {
        ...
        const typingDelay = m.isGroup ? Math.min(cleanResponse.length * 10, 1500) : 100
        await new Promise(r => setTimeout(r, typingDelay))

        if (autoai.responseType === 'voice') {
            try {
                await sock.sendPresenceUpdate('recording', m.chat)
                const { default: generateCustomTTS } = await import('../scraper/topmedia.js')
