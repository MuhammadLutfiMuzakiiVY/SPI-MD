...
function createContextInfo(jid, text, title = "", body = "", thumbnail = null) {
  ...
  if (title || body || thumbnail) {
    contextInfo.externalAdReply = {
      ...
      title: title || config.bot?.name || "Si Paling Informasi",
      body: body || "",
      mediaType: 1,
...
async function serialize(sock, msg, store = {}) {
  ...
  m.botName = sock.user?.name || config.bot?.name || "Si Paling Informasi";
  m.messageId = m.id;
  m.chatId = m.chat;
  m.senderId = m.sender;
  ...
  m.reply = async (text, options = {}) => {
        ...
        if (replyVariant === 2) {
          ...
          contextInfo = {
            ...
            forwardedNewsletterMessageInfo: {
              newsletterName: config.saluran?.name || "Si Paling Informasi",
    ...
    if (replyVariant === 4) {
      ...
      const saluranName =
        config.saluran?.name || config.bot?.name || "Si Paling Informasi";
      ...
      quotedMsg = {
        ...
        message: {
          orderMessage: {
            ...
            status: "INQUIRY",
            surface: "CATALOG",
            message: `${config.bot?.name || "Si Paling Informasi"}`,
    ...
    return sock.sendMessage(
      m.chat,
      {
        ...
        contextInfo: {
          externalAdReply: {
            showAdAttribution: true,
            title: preview.title || config.bot?.name || "Si Paling Informasi",
            body: preview.body || "",
            mediaType: 1,
            renderLargerThumbnail: true,
