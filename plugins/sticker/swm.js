...
import { addExifToWebp, isAnimatedWebp, DEFAULT_METADATA } from '../../src/lib/spi-exif.js'
...
async function handler(m, { sock, config: botConfig }) {
    ...
    if (!quoted) {
        return m.reply(
            ...
            `*ᴄᴏɴᴛᴏʜ:*\n` +
            `> \`${m.prefix}swm Si Paling Informasi\`\n` +
            `> \`${m.prefix}swm Si Paling Informasi|LuckyArchz\` _(packname + author)_`
        )
    }
    ...
    if (!input) {
        return m.reply(
            ...
            `*ᴄᴏɴᴛᴏʜ:*\n` +
            `> \`${m.prefix}swm Si Paling Informasi\`\n` +
            `> \`${m.prefix}swm Si Paling Informasi|LuckyArchz\` _(+ author)_`
        )
    }
