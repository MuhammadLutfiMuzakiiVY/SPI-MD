...
async function render(s, pf) {
  ...
  ctx.font = "10px Arial";
  ctx.fillText(
    `${config.bot?.name || "Si Paling Informasi"} • ${moment().tz("Asia/Jakarta").format("DD MMM YYYY, HH:mm:ss")} WIB`,
    30,
    56,
  );
  ctx.restore();
  ...
  ctx.textAlign = "center";
  ctx.fillText(
    `${config.bot?.name || 'Si Paling Informasi'} Performance Monitor • rendered in ${pf.canvasTime}ms`,
    W / 2,
    H - 8,
  );
