...
const pluginConfig = {
    ...
    category: 'owner',
    description: 'Menghapus semua session di storage/session/',
    usage: '.clearsessions',
...
async function handler(m)  {
    const sessionsPath = path.join(process.cwd(), 'storage', 'session')
    ...
    try {
        ...
        if (files.length === 0) {
            return m.reply(`📁 Folder session sudah kosong!`)
        }
